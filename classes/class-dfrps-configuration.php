...
if ( ! class_exists( 'Dfrps_Configuration' ) ) {
	...
	class Dfrps_Configuration {
		...
		function register_settings() {
			...
			add_settings_field( 'disable_updates_when_missing_merchants', __( 'Disable Updates When Missing Merchants', DFRPS_DOMAIN ), array( &$this, 'field_disable_updates_when_missing_merchants' ), $this->page, 'general-update' );
		...
		function field_disable_updates_when_missing_merchants() {
			?>
            <p><input type="radio" value="yes" name="<?php echo $this->key; ?>[disable_updates_when_missing_merchants]" <?php checked( $this->options['disable_updates_when_missing_merchants'], 'yes', true ); ?> /> <?php _e( 'Yes', DFRPS_DOMAIN ); ?></p>
            <p><input type="radio" value="no" name="<?php echo $this->key; ?>[disable_updates_when_missing_merchants]" <?php checked( $this->options['disable_updates_when_missing_merchants'], 'no', true ); ?> /> <?php _e( 'No', DFRPS_DOMAIN ); ?></p>
            <p class="description"><?php _e( 'Disable Product Set Updates if a Product Set is unable to update because it is missing merchants. Only applies if using the "Merchant" filter. <a href="https://datafeedrapi.helpscoutdocs.com/article/253-no-merchants-selected-error" target="_blank" rel="noopener">Learn more</a>', DFRPS_DOMAIN ); ?></p>
			<?php
		}
